...
public class ShapeL extends ShapeSuper {

    ShapeL(int x, int y, List<Integer> x_coordinate, List<Integer> y_coordinate) {
        super(x, y, x_coordinate, y_coordinate);
    }
    @Override
    public void setCoordinates() {
        x_coordinate.add((int)domainCoord.get(0,6));
        y_coordinate.add((int)domainCoord.get(1,6));
    ...
    public void rotate() {
        ...
        if(x_coordinate.get(2) < 9*DOT_SIZE && x_coordinate.get(2) > 0) {
            x_coordinate.set(0, (int)domainCoord.get(0,6) + x_coordinate.get(2)-(int)domainCoord.get(0,4)-4*DOT_SIZE);
            y_coordinate.set(0, (int)domainCoord.get(1,6) + y_coordinate.get(2)+3*DOT_SIZE);
