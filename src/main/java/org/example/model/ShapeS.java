...
public class ShapeS extends ShapeSuper {
    int i = 0;

    ShapeS(int x, int y, List<Integer> x_coordinate, List<Integer> y_coordinate) {
    ...
    public void rotate() {
        if(x_coordinate.get(2) < 9*DOT_SIZE && x_coordinate.get(2) > 0) {
            ...
            y_coordinate.set(1, y_coordinate.get(1) - (int) Math.pow(-1, i) * 2 * DOT_SIZE);
